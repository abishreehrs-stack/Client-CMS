            ...
            {users.map((u) => (
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <h3 className="font-extrabold text-sm text-on-surface leading-snug">{u.name}</h3>
                ...
                <div className="p-2.5 rounded-xl bg-surface-container/60 text-xs">
                  <span className="text-[10px] text-on-surface-variant uppercase font-bold block mb-1.5">
                    Granted Permissions
                  </span>
                  {u.role === 'SuperAdmin' || u.role === 'Admin' ? (
                    <span className="text-xs text-primary font-bold">★ Full System Access</span>
                  ) : (
                    <div className="flex flex-wrap gap-1">
                      {u.permissions?.canManageJobs && <span className="text-[10px] bg-primary/10 text-primary px-2 py-0.5 rounded-md border border-glass-border font-medium">Jobs</span>}
                      {u.permissions?.canViewApplications && <span className="text-[10px] bg-primary/10 text-primary px-2 py-0.5 rounded-md border border-glass-border font-medium">Applications</span>}
                      {u.permissions?.canManageServices && <span className="text-[10px] bg-primary/10 text-primary px-2 py-0.5 rounded-md border border-glass-border font-medium">Services</span>}
                      {u.permissions?.canManageClients && <span className="text-[10px] bg-primary/10 text-primary px-2 py-0.5 rounded-md border border-glass-border font-medium">Clients</span>}
                      {u.permissions?.canViewInquiries && <span className="text-[10px] bg-primary/10 text-primary px-2 py-0.5 rounded-md border border-glass-border font-medium">Inquiries</span>}
                    </div>
                  )}
                </div>
