...
export default function RootLayout({
  ...
  return (
    <html lang="en" className="dark" suppressHydrationWarning>
      ...
      <body className="bg-background text-on-surface antialiased min-h-screen flex selection:bg-primary-container selection:text-on-primary-container">
        ...
        {!adminUser ? (
            ...
            <div className="w-full max-w-md glass-card p-8 sm:p-10 rounded-3xl shadow-2xl relative">
              ...
              {mounted && (
                <button
                  ...
                  className="absolute top-6 right-6 p-2 rounded-xl bg-surface-container hover:bg-surface-container-high border border-glass-border text-primary transition-all shadow-sm"
              ...
              <form onSubmit={handleLogin} className="flex flex-col gap-4">
                ...
                <button
                  type="submit"
                  disabled={loggingIn}
                  className="mt-2 bg-primary hover:bg-primary-fixed-dim text-on-primary font-bold py-3.5 rounded-xl transition-all shadow-lg flex items-center justify-center gap-2 hover:scale-[1.01] active:scale-[0.99]"
                >
          ...
          <div className="flex-1 flex min-w-0">
            ...
            <aside className="w-64 bg-surface-container-low border-r border-glass-border p-6 flex flex-col justify-between hidden md:flex shrink-0 transition-colors duration-300">
                ...
                <div className="flex items-center justify-between gap-3 mb-6 pb-4 border-b border-glass-border">
                  ...
                  {mounted && (
                    <button
                      onClick={toggleTheme}
                      aria-label="Toggle Theme"
                      className="p-2 rounded-lg bg-surface-container hover:bg-surface-container-high border border-glass-border text-primary transition-all shadow-sm"
                      title={isDarkTheme ? 'Switch to Light Theme' : 'Switch to Dark Theme'}
                    >
              ...
              <div className="pt-6 border-t border-glass-border flex flex-col gap-3">
                <a
                  ...
                  className="w-full bg-surface-container hover:bg-surface-container-high border border-glass-border text-primary text-xs font-bold py-2.5 rounded-xl transition-all flex items-center justify-center gap-1.5"
                >
                ...
                <button
                  ...
                  className="w-full bg-tertiary/10 hover:bg-tertiary/20 border border-tertiary/30 text-tertiary text-xs font-bold py-2.5 rounded-xl transition-all flex items-center justify-center gap-1.5"
                >
            ...
            {/* Main Workspace */}
            <main className="flex-1 p-6 sm:p-10 overflow-y-auto bg-surface transition-colors duration-300">
              {children}
            </main>
          </div>
        )}
